...
// POST /profile/upgrade — bị vô hiệu hóa: chỉ admin mới cấp Pro qua trang /admin
app.post('/profile/upgrade', async (req, res) => {
  if (!req.session.userId) return res.redirect('/auth/login');
  // Nếu là admin tự bấm, cho phép toggle (demo)
  if (req.session.role === 'admin') {
    ...
    return res.redirect('/profile?flash=Cập+nhật+gói+thành+công&type=success');
  }
  // User thường: không được tự nâng cấp
  res.redirect('/pricing?flash=Vui+lòng+liên+hệ+Admin+để+được+cấp+quyền+Pro&type=error');
});
